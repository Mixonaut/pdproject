...
const smartHomeApi = {
  ...
  energy: {
    ...
    getSummary: async function(roomId, period = 'day') {
      try {
        const response = await fetch(`/api/rooms/${roomId}/energy/summary?period=${period}`);
        if (!response.ok) throw new Error('Failed to fetch energy summary');
        return await response.json();
      } catch (error) {
        console.error(`Error fetching energy summary for room ${roomId}:`, error);
        return null;
      }
    },
    ...
    getComparison: async function(roomId, period = 'day') {
      try {
        const response = await fetch(`/api/rooms/${roomId}/energy/comparison?period=${period}`);
        if (!response.ok) throw new Error('Failed to fetch energy comparison');
        return await response.json();
      } catch (error) {
        console.error(`Error fetching energy comparison for room ${roomId}:`, error);
        return null;
      }
    },
    ...
    getByDeviceType: async function(roomId, period = 'day') {
      try {
        const response = await fetch(`/api/rooms/${roomId}/energy/by-device-type?period=${period}`);
        if (!response.ok) throw new Error('Failed to fetch energy by device type');
        return await response.json();
      } catch (error) {
        console.error(`Error fetching energy by device type for room ${roomId}:`, error);
        return [];
      }
    }
  },
  ...
  test: {
    ...
    generateEnergyData: async function(roomId, deviceId, count = 24) {
      try {
        const response = await fetch(`/api/test/rooms/${roomId}/devices/${deviceId}/energy`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        if (!response.ok) throw new Error('Failed to generate test energy data');
        return await response.json();
      } catch (error) {
        console.error(`Error generating test energy data for device ${deviceId}:`, error);
        throw error;
      }
    }
  }
